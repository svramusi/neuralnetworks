...
public abstract class AparapiSubsampling2D extends Kernel implements ConnectionCalculator {
    ...
    /**
     * Contains the offset in the input array for each cell of the current
     * region. The offset is calculated in respect to the first cell of the
     * region
     */
    // @Local TODO
     ...
     * (non-Javadoc)
     * 
     * @see com.amd.aparapi.Kernel#run() input start index is calculated and
     * passed to the pooling method
     */
    @Override
    public void run() {
        int id = getGlobalId();
        ...
        pool((id / outputFeatureMapLength) * inputFeatureMapLength + (ioRowsOffset + (fmOffset / outputFeatureMapColumns) * subsamplingRows) * inputFeatureMapColumns
                + ioColumnsOffset + (fmOffset % outputFeatureMapColumns) * subsamplingCols);
    }
