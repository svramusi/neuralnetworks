...
public class TargetLayerOrderStrategy implements LayerOrderStrategy {
     ...
     * 
     * For example the feedforward part of the backpropagation algorithm the
     * initial parameters would be: "currentLayer" will be the output layer of
     * the network "results" will contain only one entry for the input layer of
     * the network - this is the training example "calculatedLayers" will
     * contain only one entry - the input layer "inProgressLayers" will be empty
     * 
     * In the backpropagation part the initial parameters would be:
     * "currentLayer" will be the input layer of the network "results" will
     * contain only one entry for the output layer of the network - this is the
     * calculated error derivative between the result of the network and the
     * target value "calculatedLayers" will contain only one entry - the output
     * layer "inProgressLayers" will be empty
     * 
     * This allows for single code to be used for the whole backpropagation, but
     * also for RBMs, autoencoders, etc
     * 
     * @param neuralNetwork
     *            - the neural network.
     * @param calculatedLayers
     *            - layers that are fully calculated - the results for these
     *            layers can be used for calculating other parts of the network
     * @param inProgressLayers
     *            - layers which are currently calculated, but are not yet
     *            finished - not all connections to the layer are calculated and
     *            the result of the propagation through this layer cannot be
     *            used for another calculations
     * @param calculateCandidates
     *            - order of calculation
     * @param currentLayer
     *            - the layer which is currently being calculated.
     * @return
     */
    protected boolean orderConnections(NeuralNetwork neuralNetwork, Layer currentLayer, Set<Layer> calculatedLayers, Set<Layer> inProgressLayers,
            List<ConnectionCandidate> calculateCandidates) {
